...
#   4. BOM rendered from umbrella-release.yaml.tmpl + notes committed as the
#      BOM commit (C_bom); image digests are pinned by release-umbrella CI
#      (sha256:PENDING until then)
...
Options:
  ...
  --channel <stable|rc>     Defaults to rc when the version has a suffix, else stable.
                            rc renders umbrella-release-rc.yaml.tmpl (no package sections).
...
# ---------------------------------------------------------------------------
# BOM — rendered from the channel template (stable / rc variants)
# ---------------------------------------------------------------------------

RELEASES_DIR="docs/releases"
BOM_FILE="${RELEASES_DIR}/${VERSION}.yaml"
NOTES_OUT_FILE="${RELEASES_DIR}/${VERSION}.md"
TEMPLATE_FILE="${RELEASE_DIR}/umbrella-release.yaml.tmpl"
[[ "$CHANNEL" == "rc" ]] && TEMPLATE_FILE="${RELEASE_DIR}/umbrella-release-rc.yaml.tmpl"
[[ -f "$TEMPLATE_FILE" ]] || die "BOM template not found: ${TEMPLATE_FILE}"
spec_lifecycle_sha="$(sha256_of specs/sandbox-lifecycle.yml)"
spec_execd_sha="$(sha256_of specs/execd-api.yaml)"
spec_egress_sha="$(sha256_of specs/egress-api.yaml)"
spec_diagnostic_sha="$(sha256_of specs/diagnostic-api.yml)"

BOM_FILE_TMP="$(mktemp -t opensandbox-bom.XXXXXX.yaml)"
# Placeholder tokens are plain ${VAR} strings; bracket-style regex classes
# ([$][{]VAR[}]) keep the substitution portable across awk variants. When a
# digests manifest is supplied, image entries are pinned in the same pass —
# each PENDING digest is matched by the basename of the preceding image ref.
DIGESTS_TSV=""
if [[ -n "$DIGESTS_MANIFEST" ]]; then
  require_cmd jq
  DIGESTS_TSV="$(mktemp -t opensandbox-digests.XXXXXX)"
  jq -r 'to_entries[] | "\(.key)\t\(.value)"' "$DIGESTS_MANIFEST" >"$DIGESTS_TSV"
fi
awk \
  -v manifest="$DIGESTS_TSV" \
  -v version="$VERSION" \
  -v line="$LINE" \
  -v channel="$CHANNEL" \
  -v release_date="$BUILD_DATE" \
  -v git_commit="$C_BUILD" \
  -v spec_lifecycle="$spec_lifecycle_sha" \
  -v spec_execd="$spec_execd_sha" \
  -v spec_egress="$spec_egress_sha" \
  -v spec_diagnostic="$spec_diagnostic_sha" \
  '
  BEGIN {
    if (manifest != "") {
  ...
  {
    if ($1 == "image:") { n = split($2, seg, "/"); cur = seg[n] }
    if ($1 == "digest:" && $2 == "sha256:PENDING" && cur != "" && (cur in map)) {
      sub(/sha256:PENDING/, map[cur])
    }
    ...
    gsub(/[$][{]SPEC_SANDBOX_LIFECYCLE_SHA256[}]/, spec_lifecycle)
    gsub(/[$][{]SPEC_EXECD_API_SHA256[}]/, spec_execd)
    gsub(/[$][{]SPEC_EGRESS_API_SHA256[}]/, spec_egress)
    gsub(/[$][{]SPEC_DIAGNOSTIC_API_SHA256[}]/, spec_diagnostic)
    print
  }' "$TEMPLATE_FILE" >"$BOM_FILE_TMP"
[[ -n "$DIGESTS_TSV" ]] && rm -f "$DIGESTS_TSV"
