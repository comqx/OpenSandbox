...
| `EXECD_ENVS` | Optional file of `KEY=VALUE` entries supplying environment variables for commands and bash sessions. Blank lines and `#` comment lines are ignored. Unquoted values are trimmed and expand daemon environment variables (`$NAME`, `${NAME}`). Double-quoted values keep whitespace, honor `\n` `\r` `\t` `\\` `\"` escapes, still expand variables, and may span lines. Single-quoted values are fully literal — no expansion, no escapes, whitespace and newlines kept verbatim — and are the lossless form for writers (`KEY='value'` always parses back verbatim; values containing `'` must use double quotes with escapes, where `$` references still expand). Malformed and unterminated entries are skipped with a warning. Bash session `cwd` values also expand `$NAME` and `${NAME}` using the session environment. |
